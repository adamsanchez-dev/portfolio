...
public class Guerrer extends Personatge {
    ...
    public Guerrer(String nom, String classe, int forca, int cons, int vel, int inte, int sort) {
        super(nom, classe, forca, cons, vel, inte, sort);
        calculaDerivades();
    }

    public Guerrer(String nom, String classe, int forca, int cons, int vel, int inte, int sort, Arma arma) {
        super(nom, classe, forca, cons, vel, inte, sort, arma);
        calculaDerivades();
    }
    public Guerrer(String nom, String classe, int forca, int cons, int vel, int inte, int sort, Arma arma, int pex, int lvl) {
    ...
    public void calculaDerivades() {
        ...
        this.pd = (forca + arma.getWpow() + constitucio) / 4;
